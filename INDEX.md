...
- **frontend/**
  ...
  - **js/**
    ...
    - **pages/**
      ...
      - user-profile.js — user-profile.html: public profile view, progress/achievements/favorites, friend-request controls; adds a "share profile" button and a read-only seller-rating stat tile (`loadSellerRatingSummary()`, only shown if the user has seller reviews); the "Active listings" grid (`loadUserListings()`) shows a "Restored by CNote" badge for official listings
