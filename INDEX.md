...
- **backend/**
  ...
  - **routes/**
    ...
    - console-tutorials.js — `/api/console-tutorials/:code`: per-model disassembly-tutorial content for console-model.html, keyed by the same `code` as `frontend/js/data/console-models.js`. `GET` is public (no auth); `PUT` (upsert) and `DELETE` are `authRequired` + `adminOnly`. Self-creates `console_tutorials` table on load (model_code PK, title, intro, steps JSONB `[{heading, description, image_url}]`, updated_by/updated_at) — same self-init pattern as courses.js's `lesson_reactions`. Step photos go through `POST /api/uploads/presign` with a new `kind: 'tutorial'` (admin-gated there too, see uploads.js).
    ...
    - uploads.js — presigned PUT URL issuance for chat image/voice/community-gallery/tutorial attachments (`kind: 'image'|'voice'|'gallery'|'tutorial'`; file bytes never touch this server); `kind: 'tutorial'` additionally requires `req.user.role === 'admin'` (console-model.html's write feature — see console-tutorials.js)
...
- **frontend/**
  ...
  - **html/**
    ...
    - **pages/**
      ...
      - console-care.html — static, public "Console Care & Cleaning Guide" page (no login required), own design (`console-care.css`, not the legal-docs `.terms-page` look). Order: hero (with 3 decorative `.model-plate` mockups) → "Identify Your Model" directory (`#care-directory-grid`, searchable/filterable by manufacturer, populated by `console-care.js` from real researched PlayStation/Xbox/Nintendo hardware model numbers, 194 entries incl. individual regional/revision variants, e.g. PS2 `SCPH-50004`, PS2 `SCPH-30000R` (a distinct board revision, not just a region code — reversed power/reset switch wiring), PS3 `CECHC04` — each plate is a clickable `<a>` to `console-model.html?code=<code>`) → a pill chip-nav → maintenance topics as `.care-topic` panel cards (vents/dust, cartridge contacts, optical drive/laser lens care, controller stick drift, safe opening, storage/transport, glossary, CTA into the Community repair-request wizard `community.html#repair-requests`) using `lesson.css`'s callout components (`think-block`/`myth-block`/`glossary-block`/`cta-block`) → a short "Quick Tips" checklist (`.care-tips`, 7 one-liners) as the final section. Static Article+BreadcrumbList JSON-LD in `<head>`; all body copy is `data-i18n`'d (`care_*` keys, all 6 languages) — section headings are plain topic labels, not numbered, since these are independent reference topics rather than a sequence. Linked from a 4th card on invata.html's course grid and from a new FAQ entry (`help_con_q8`/`a8`) on help-console.html; listed in both sitemap.xml copies. Verified with a real headless-browser render (default/dark/light themes) before commit — caught and fixed a wrong `../../js/components.js` script path (should be `../js/components.js`, one level up from `html/pages/`) that had silently broken navbar/footer injection since the page was first created.
      - console-model.html — dynamic per-model detail page, `?code=<code>` query param (e.g. `?code=SCPH-50004`), no static per-model files (same query-string pattern as comparatie.html). Reads `frontend/js/data/console-models.js` via `console-model.js`, shows the model's plate + manufacturer/console/note, plus the disassembly tutorial content (title/intro/numbered steps with photos) fetched from `GET /api/console-tutorials/:code`, or a "Coming soon" placeholder if none has been written yet. Admin-only (`AuthModule.getCurrentUser()?.role === 'admin'`) sees a "Write/Edit tutorial" button that opens an inline editor (title, intro, add/remove steps each with heading + description + photo upload) saving via `PUT /api/console-tutorials/:code`, plus a "Delete tutorial" option reverting to "Coming soon". `<meta name="robots" content="noindex, follow">` until real tutorial content exists (avoids indexing thin placeholder pages) — **note**: this stays static regardless of whether a tutorial has been written; revisit once real per-model content exists broadly (removing noindex, and considering sitemap inclusion). Falls back to a "model not found" message + link back to the directory if `?code=` doesn't match anything.
  ...
  - **js/**
    ...
    - **pages/**
      ...
      - console-care.js — console-care.html: renders + filters the "Identify Your Model" hardware directory from `MODELS` (imported from `../data/console-models.js`) into `.model-plate` cards grouped by console, each a clickable `<a href="console-model.html?code=...">`; text search box and PlayStation/Xbox/Nintendo tab filter; no build step, plain DOM rendering; imports `I18nModule` from `modules/i18n.js` to translate the "Model No." plate label and re-renders on `cn:language-changed`
      - console-model.js — console-model.html: reads `?code=` from the URL, looks it up in the shared `MODELS` data, fills in the plate/title/note, sets `<title>`/meta description dynamically; shows a "model not found" fallback if the code doesn't match. Fetches/renders the tutorial from `GET /api/console-tutorials/:code`; for admins, builds an inline step-by-step editor (dynamic `.tutorial-editor__step` rows, each wired to `POST /api/uploads/presign` `kind: 'tutorial'` → direct PUT to storage, mirroring community.js's `submitPhotoUpload` flow) and saves via `PUT`/`DELETE /api/console-tutorials/:code`
