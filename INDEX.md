...
- **frontend/**
  ...
  - **html/**
    ...
    - **pages/**
      ...
      - console-model.html — dynamic per-model detail page, `?code=<code>` query param (e.g. `?code=SCPH-50004`), no static per-model files (same query-string pattern as comparatie.html). Reads `frontend/js/data/console-models.js` via `console-model.js`, shows the model's plate + manufacturer/console/note, and a "Coming soon" placeholder for the future photo disassembly tutorial. `<meta name="robots" content="noindex, follow">` until real tutorial content exists (avoids indexing thin placeholder pages). Falls back to a "model not found" message + link back to the directory if `?code=` doesn't match anything.
  ...
  - **js/**
    ...
    - **pages/**
      ...
      - console-model.js — console-model.html: reads `?code=` from the URL, looks it up in the shared `MODELS` data, fills in the plate/title/note, sets `<title>`/meta description dynamically; shows a "model not found" fallback if the code doesn't match
