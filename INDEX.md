...
- **frontend/**
  ...
  - **html/**
    ...
    - **pages/**
      ...
      - console-care.html — static, public "Console Care & Cleaning Guide" page (no login required), own design (`console-care.css`, not the legal-docs `.terms-page` look). Order: hero (with 3 decorative `.model-plate` mockups) → "Identify Your Model" directory (`#care-directory-grid`, searchable/filterable by manufacturer, populated by `console-care.js` from real researched PlayStation/Xbox/Nintendo hardware model numbers, e.g. PS2 `SCPH-50004`, PS3 `CECHC04`) → a pill chip-nav → maintenance topics as `.care-topic` panel cards (vents/dust, cartridge contacts, optical drive/laser lens care, controller stick drift, safe opening, storage/transport, glossary, CTA into the Community repair-request wizard `community.html#repair-requests`) using `lesson.css`'s callout components (`think-block`/`myth-block`/`glossary-block`/`cta-block`). Static Article+BreadcrumbList JSON-LD in `<head>`; all body copy is `data-i18n`'d (`care_*` keys, all 6 languages) — section headings are plain topic labels, not numbered, since these are independent reference topics rather than a sequence. Linked from a 4th card on invata.html's course grid and from a new FAQ entry (`help_con_q8`/`a8`) on help-console.html; listed in both sitemap.xml copies.
  ...
  - **js/**
    ...
    - **pages/**
      ...
      - console-care.js — console-care.html: renders + filters the "Identify Your Model" hardware directory from a flat `MODELS` array (`{mfr, console, code, note}`, 112 researched entries — PlayStation/Xbox/Nintendo home consoles and handhelds) into `.model-plate` cards grouped by console, with a text search box and PlayStation/Xbox/Nintendo tab filter; no build step, plain DOM rendering; imports `I18nModule` from `modules/i18n.js` to translate the "Model No." plate label and re-renders on `cn:language-changed`
