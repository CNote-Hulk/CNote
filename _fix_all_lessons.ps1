...
# Model: lectia-1-1.html structure and tone
$basePath = "c:\Users\andre\OneDrive\Documentos\Proiecte HTML\Atestat Info\src\html\pages\curs"
$utf8NoBom = New-Object System.Text.UTF8Encoding $false

# --- INTRO DATA: [hook, topics, purpose] for each lesson ---
$intros = [ordered]@{}

$intros["lectia-1-2.html"] = @(
...
$intros["lectia-3-3.html"] = @(
"Porțile logice procesează date, dar nu le pot reține — output-ul dispare imediat ce input-ul se schimbă. Pentru a "memora" un bit, avem nevoie de un circuit care își păstrează starea: flip-flop-ul. Fiecare registru din procesorul unui PlayStation 5 și fiecare celulă din memoria cache L1 se bazează pe acest principiu: un circuit bistabil care stochează un 0 sau un 1 până când este comandat explicit să se schimbe.",
...
"Scopul nu este memorarea pragurilor de temperatură, ci înțelegerea <em>de ce</em> puterea disipată crește cu frecvența și cu pătratul tensiunii (P ∝ f × V²), <em>cum</em> consolele moderne redistribuie bugetul de putere dinamic între CPU și GPU și <em>de ce</em> o consolă cu pasta termică uscată pierde performanță înainte de a se opri."
)

$intros["lectia-7-5.html"] = @(
...
# --- SECTION TEMPLATE ---
function Build-IntroSection($hook, $topics, $purpose) {
    return @"
...
foreach ($entry in $intros.GetEnumerator()) {
    ...
    # Step 1: Fix HTML entities → UTF-8
    ...
    $newSection = Build-IntroSection $entry.Value[0] $entry.Value[1] $entry.Value[2]
    ...
    if ($introMatch.Success) {
        ...
        Write-Host "WARN: $($entry.Key) - intro section not matched"
    }

    # Step 3: Write only if changed
    if ($content -ne $original) {
        [System.IO.File]::WriteAllText($filePath, $content, $utf8NoBom)
        $entities = [regex]::Matches($original, $entityPattern).Count
        if ($entities -gt 0) { $entityCount += $entities }
        Write-Host "OK: $($entry.Key) (entities fixed: $entities)"
    } else {
        Write-Host "UNCHANGED: $($entry.Key)"
    }
}

Write-Host "`nIntros updated: $count / $($intros.Count)"
Write-Host "Total entities fixed: $entityCount"
