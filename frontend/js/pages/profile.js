...
document.addEventListener('DOMContentLoaded', function () {
        ...
        function setActive(tabKey) {
            document.querySelectorAll('.mbn-item[data-mbn-tab]').forEach(function (el) {
                el.classList.toggle('mbn-item--active', el.dataset.mbnTab === tabKey);
            });
            document.querySelectorAll('.mbn-dd-item[data-mbn-tab]').forEach(function (el) {
                var match = el.dataset.mbnTab === tabKey;
        ...
        function switchTab(tabKey) {
            var profileBtn = document.querySelector('.profile-tab[data-tab="' + tabKey + '"]');
            if (profileBtn) profileBtn.click();
            setActive(tabKey);
        }

        var initial = location.hash.slice(1);
        setActive(TABS.indexOf(initial) >= 0 ? initial : 'account');

        document.querySelectorAll('.mbn-item[data-mbn-tab], .mbn-dd-item[data-mbn-tab]').forEach(function (el) {
