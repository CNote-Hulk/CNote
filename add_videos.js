...
const BASE = 'c:/Users/andre/OneDrive/Documentos/CNote/frontend/html/pages/consoles';
...
function makeSection(vid, title, desc) {
    return `
    <!-- ===========================
...
for (const [filename, [vid, title, desc]] of Object.entries(VIDEOS)) {
    const filepath = path.join(BASE, filename);
    if (!fs.existsSync(filepath)) { skipped.push('NOT FOUND: ' + filename); continue; }

    let content = fs.readFileSync(filepath, 'utf8');

    if (content.includes('console-video-section')) { skipped.push('ALREADY HAS VIDEO: ' + filename); continue; }

    let idx = content.indexOf('<!-- ===========================\n         FOOTER');
    if (idx === -1) idx = content.indexOf('<footer');
    if (idx === -1) { skipped.push('NO FOOTER: ' + filename); continue; }

    content = content.slice(0, idx) + makeSection(vid, title, desc) + content.slice(idx);
